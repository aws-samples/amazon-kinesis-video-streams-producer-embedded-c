...
#ifdef KVS_WITH_WEBRTC
#include "frame_ring_buffer/frame_ring_buffer.h"
#endif /* #ifdef KVS_WITH_WEBRTC */

extern PSampleConfiguration gSampleConfiguration;

// #define VERBOSE

INT32 webrtc_main(INT32 argc, CHAR* argv[])
{
...
#ifdef KVS_WITH_WEBRTC
    /* SIGINT is handled in "kvs_with_webrtc.c". */
#else
#ifndef _WIN32
...
#ifdef KVS_WITH_WEBRTC
    /* Media source is fed from "kvs_with_webrtc.c". */
#else
...
#ifdef KVS_WITH_WEBRTC
    /* Media source is fed from "kvs_with_webrtc.c", so we don't need to check the media source here. */
#else
...
#ifdef KVS_WITH_WEBRTC
/* Media source is fed from "kvs_with_webrtc.c", so we don't need to the source feeder here. */
#else
...
#ifdef KVS_WITH_WEBRTC
static int webrtc_onRtpResend(uint8_t **ppData, size_t *puLen, void *pAppData)
{
    int res = 0;
    FrameKeyHandle keyHandle = (FrameKeyHandle)pAppData;

    res = FrameRingBuffer_getFrame(keyHandle, ppData, puLen);
...
int webrtc_taskAddVideoFrame(uint8_t *pData, size_t uLen, uint64_t uTimestamp, FrameKeyHandle keyHandle)
{
    ...
    if (pSampleConfiguration != NULL)
    {
        ...
        frameEx.onRtpResend = webrtc_onRtpResend;
        frameEx.pAppData = keyHandle;

        MUTEX_LOCK(pSampleConfiguration->streamingSessionListReadLock);
        for (i = 0; i < pSampleConfiguration->streamingSessionCount; ++i) {
            status = writeFrameEx(pSampleConfiguration->sampleStreamingSessionList[i]->pVideoRtcRtpTransceiver, &frame, &frameEx);
        }
        MUTEX_UNLOCK(pSampleConfiguration->streamingSessionListReadLock);
    }
