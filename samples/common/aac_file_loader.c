...
#define CODEC_NAME "A_AAC"

typedef struct AacFileLoader
{
    char *pcTrackName;
    char *pcFileFormat;
...
static int loadFrame(AacFileLoader_t *pLoader, char **ppData, size_t *puDataLen)
{
    int res = ERRNO_NONE;
...
static int initializeAudioTrackInfo(AacFileLoader_t *pLoader, Mpeg4AudioObjectTypes_t xObjectType, uint32_t uFrequency, uint16_t uChannelNumber)
{
    int res = ERRNO_NONE;
    ...
    else
    {
        ...
        if (Mkv_generateAacCodecPrivateData(xObjectType, uFrequency, uChannelNumber, &pCodecPrivateData, &uCodecPrivateDataLen) == 0)
        {
...
{
    AacFileLoader_t *pLoader = xLoader;
