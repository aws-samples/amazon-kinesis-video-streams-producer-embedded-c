...
#ifdef KVS_RECORD_SESSION
STATUS getRtpRollingBufferIndex(PRtcRtpTransceiver pRtcRtpTransceiver, PUINT64 pHeadIndex, PUINT64 pTailIndex)
{
    STATUS retStatus = STATUS_SUCCESS;
    PKvsRtpTransceiver pKvsRtpTransceiver = (PKvsRtpTransceiver) pRtcRtpTransceiver;
    PRtpRollingBuffer packetBuffer = NULL;

    CHK(pKvsRtpTransceiver != NULL && pHeadIndex != NULL && pTailIndex != NULL, STATUS_NULL_ARG);
    packetBuffer = pKvsRtpTransceiver->sender.packetBuffer;

    *pHeadIndex = packetBuffer->pRollingBuffer->headIndex;
    *pTailIndex = packetBuffer->pRollingBuffer->tailIndex;
...
STATUS getRtpRollingBufferData(PRtcRtpTransceiver pRtcRtpTransceiver, UINT64 index, PBYTE pRawPacketBuffer, UINT32 uRawPacketBufferSize, PUINT32 puRawPacketLength)
{
    STATUS retStatus = STATUS_SUCCESS;
    PKvsRtpTransceiver pKvsRtpTransceiver = (PKvsRtpTransceiver) pRtcRtpTransceiver;
    PRtpRollingBuffer packetBuffer = NULL;
    ...
    UINT32 uRawPacketLength = 0;

    CHK(pKvsRtpTransceiver != NULL && pRawPacketBuffer != NULL && uRawPacketBufferSize > 0 && puRawPacketLength != NULL, STATUS_NULL_ARG);

    packetBuffer = pKvsRtpTransceiver->sender.packetBuffer;
    CHK(index < packetBuffer->pRollingBuffer->headIndex && index >= packetBuffer->pRollingBuffer->tailIndex, STATUS_INVALID_ARG);

    rollingBufferExtractData(packetBuffer->pRollingBuffer, index, &item);
    pRtpPacket = (PRtpPacket) item;
    CHK(pRtpPacket != NULL, STATUS_INTERNAL_ERROR);

    uRawPacketLength = pRtpPacket->rawPacketLength;
    CHK(uRawPacketLength <= uRawPacketBufferSize, STATUS_INVALID_STORAGE_SIZE);
    memcpy(pRawPacketBuffer, pRtpPacket->pRawPacket, uRawPacketLength);
    isEncrypted = (pKvsRtpTransceiver->sender.payloadType == pKvsRtpTransceiver->sender.rtxPayloadType);

    if (isEncrypted)
    {
        // pKvsRtpTransceiver->pKvsPeerConnection->pSrtpSession->srtp_record_session
        CHK_STATUS(decryptSrtpRecord(pKvsRtpTransceiver->pKvsPeerConnection->pSrtpSession, pRawPacketBuffer, &uRawPacketLength));
    }

    *puRawPacketLength = uRawPacketLength;
...
#if 0
STATUS getRtpRollingBuffer(PRtcRtpTransceiver pRtcRtpTransceiver, void **ppPacketBuffer)
{
    STATUS retStatus = STATUS_SUCCESS;
    PKvsRtpTransceiver pKvsRtpTransceiver = (PKvsRtpTransceiver) pRtcRtpTransceiver;
    PRtpRollingBuffer packetBuffer = NULL;

    CHK(pKvsRtpTransceiver != NULL && ppPacketBuffer != NULL, STATUS_NULL_ARG);
    packetBuffer = pKvsRtpTransceiver->sender.packetBuffer;
    *ppPacketBuffer = packetBuffer;
...
BOOL isRtpRollingBufferEncrypted(PRtcRtpTransceiver pRtcRtpTransceiver)
{
    PKvsRtpTransceiver pKvsRtpTransceiver = (PKvsRtpTransceiver) pRtcRtpTransceiver;
    BOOL isEncrypted = FALSE;
...
STATUS printRtpRollingBufferStatus(PVOID pRollingBufferPara)
{
    STATUS retStatus = STATUS_SUCCESS;
    PRtpRollingBuffer pRollingBuffer = (PRtpRollingBuffer)pRollingBufferPara;

    CHK(pRollingBuffer != NULL, STATUS_NULL_ARG);

    printf("lastIndex:%" PRIu64 " headIndex:%" PRIu64 " tailIndex:%" PRIu64 "\n", pRollingBuffer->lastIndex, pRollingBuffer->pRollingBuffer->headIndex, pRollingBuffer->pRollingBuffer->tailIndex);
...
STATUS getRtpRollingBufferIndex(PVOID pRollingBufferPara, PUINT64 pHeadIndex, PUINT64 pTailIndex)
{
    STATUS retStatus = STATUS_SUCCESS;
    PRtpRollingBuffer pRollingBuffer = (PRtpRollingBuffer)pRollingBufferPara;
    UINT64 lastIndex = 0;

    CHK(pRollingBuffer != NULL && pHeadIndex != NULL && pTailIndex != NULL, STATUS_NULL_ARG);

    *pHeadIndex = pRollingBuffer->pRollingBuffer->headIndex;
    *pTailIndex = pRollingBuffer->pRollingBuffer->tailIndex;
...
STATUS getRtpRollingBufferData(PVOID pRollingBufferPara, UINT64 index, PBYTE *ppRawPacket, PUINT32 pRawPacketLength)
{
    STATUS retStatus = STATUS_SUCCESS;
    PRtpRollingBuffer pRollingBuffer = (PRtpRollingBuffer)pRollingBufferPara;
    UINT64 item;
    ...
    CHK(pRollingBuffer != NULL && ppRawPacket != NULL && pRawPacketLength != NULL, STATUS_NULL_ARG);

    rollingBufferExtractData(pRollingBuffer, index, &item);
    pRtpPacket = (PRtpPacket) item;

    *ppRawPacket = pRtpPacket->pRawPacket;
    *pRawPacketLength = pRtpPacket->rawPacketLength;
