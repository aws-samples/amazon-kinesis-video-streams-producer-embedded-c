...
#ifdef KVS_RECORD_SESSION
    MEMSET(&recordPolicy, 0x00, SIZEOF(srtp_policy_t));
#endif
...
#ifdef KVS_RECORD_SESSION
    srtp_policy_setter(&recordPolicy.rtp);
    srtcp_policy_setter(&recordPolicy.rtcp);

    recordPolicy.key = transmitKey;
    recordPolicy.ssrc.type = ssrc_any_outbound;
    recordPolicy.next = NULL;

    CHK_ERR((errStatus = srtp_create(&(pSrtpSession->srtp_record_session), &recordPolicy)) == srtp_err_status_ok,
            STATUS_SRTP_TRANSMIT_SESSION_CREATION_FAILED, "Create srtp session for the recorder failed with error code %u", errStatus);
#endif
...
#ifdef KVS_RECORD_SESSION
    if ((pSrtpSession->srtp_record_session != NULL) && (errStatus = srtp_dealloc(pSrtpSession->srtp_record_session)) != srtp_err_status_ok) {
        DLOGW("Dealloc of record session failed with error code %d\n", errStatus);
    }
#endif
...
#ifdef KVS_RECORD_SESSION
STATUS decryptSrtpRecord(PSrtpSession pSrtpSession, PVOID encryptedMessage, PINT32 len)
{
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
    srtp_err_status_t errStatus;

    CHK_ERR((errStatus = srtp_unprotect(pSrtpSession->srtp_record_session, encryptedMessage, len)) == srtp_err_status_ok, STATUS_SRTP_DECRYPT_FAILED,
            "Decrypting rtp record failed with error code %u on srtp session %" PRIu64, errStatus, pSrtpSession->srtp_record_session);
...
STATUS decryptSrtcpRecord(PSrtpSession pSrtpSession, PVOID encryptedMessage, PINT32 len)
{
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
    srtp_err_status_t errStatus;

    CHK_ERR((errStatus = srtp_unprotect_rtcp(pSrtpSession->srtp_record_session, encryptedMessage, len)) == srtp_err_status_ok,
            STATUS_SRTP_DECRYPT_FAILED, "Decrypting rtcp record failed with error code %u on srtp session %" PRIu64, errStatus,
            pSrtpSession->srtp_record_session);
