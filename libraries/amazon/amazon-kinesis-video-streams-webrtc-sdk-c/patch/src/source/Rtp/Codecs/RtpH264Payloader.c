...
static BOOL isNalueAnnexbFrame(PBYTE nalus, UINT32 nalusLength)
{
    if (nalusLength >= 3 && nalus[0] == 0x00 && nalus[1] == 0x00 && nalus[2] == 0x01) {
        return TRUE;
    } else if (nalusLength >= 4 && nalus[0] == 0x00 && nalus[1] == 0x00 && nalus[2] == 0x00 && nalus[3] == 0x01) {
        return TRUE;
    }
    return FALSE;
}

STATUS getNextNaluLengthEx(PBYTE nalus, UINT32 nalusLength, PUINT32 pStart, PUINT32 pNaluLength, BOOL isAnnexbFrame)
{
    ENTERS();
    ...
    if (isAnnexbFrame) {
        retStatus = getNextNaluLength(nalus, nalusLength, pStart, pNaluLength);
    } else {
        CHK(nalus != NULL && nalusLength >= 4 && pStart != NULL && pNaluLength != NULL, STATUS_NULL_ARG);

        naluLength = nalus[0];
        naluLength = (naluLength << 8) | nalus[1];
        naluLength = (naluLength << 8) | nalus[2];
        naluLength = (naluLength << 8) | nalus[3];
