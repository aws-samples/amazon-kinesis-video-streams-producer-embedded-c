                            ...
                            PUINT32 pPayloadSubLength, PUINT32 pPayloadSubLenSize)
{
    ...
    PUINT32 payloadRefOffset = pPayloadSubLength + (*pPayloadSubLenSize);
    PUINT32 payloadRefLength = pPayloadSubLength + (*pPayloadSubLenSize) * 2;
#endif
    ...
    for (remainingLength = g711FrameLength; remainingLength > mtu; remainingLength -= mtu, pCurSubLen++) {
        *pCurSubLen = mtu;
#ifdef SUPPORT_SHARE_BUFFER
        *payloadRefOffset = currentOffset;
        *payloadRefLength = mtu;
        currentOffset += mtu;
        payloadRefOffset++;
        payloadRefLength++;
#endif
...
#ifdef SUPPORT_SHARE_BUFFER
    *payloadRefOffset = currentOffset;
    *payloadRefLength = remainingLength;
