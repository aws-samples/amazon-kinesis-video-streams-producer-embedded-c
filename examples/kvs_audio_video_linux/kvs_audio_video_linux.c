...
typedef struct
{
    ...
    bool isFirstFrame;
    bool isKeyFrame;
    uint64_t videoFrameTimestamp;
    uint64_t audioFrameTimestamp;
    uint64_t lastKeyFrameTimestamp;
} StreamData_t;

static int videoFileIndex = 0;
...
static int loadFrame( StreamData_t * pStreamData )
{
    ...
    uint64_t timeToNextVideoFps = 0;
    uint64_t timeToNextAudioFps = 0;
    timestamp = getEpochTimestampInMs();

    if( ( timestamp - pStreamData->videoFrameTimestamp ) < ( 1000 / VIDEO_FPS ) && ( timestamp - pStreamData->audioFrameTimestamp ) < ( 1000 / AUDIO_FPS ) )
    {
        timeToNextVideoFps = ( 1000 / VIDEO_FPS ) - ( timestamp - pStreamData->videoFrameTimestamp );
        timeToNextAudioFps = ( 1000 / AUDIO_FPS ) - ( timestamp - pStreamData->audioFrameTimestamp );
        if( timeToNextVideoFps <= timeToNextAudioFps )
        {
            sleepInMs( timeToNextVideoFps );
        }
        else
        {
            sleepInMs( timeToNextAudioFps );
        }
    }
    timestamp = getEpochTimestampInMs();

    if( ( timestamp - pStreamData->videoFrameTimestamp ) >= ( 1000 / VIDEO_FPS ) )
    {
        pStreamData->videoFrameTimestamp = getEpochTimestampInMs();
        pStreamData->trackId = MKV_VIDEO_TRACK_NUMBER;

        /* Load frame from file */
        xFilenameSize = snprintf( NULL, 0, "%s/" SAMPLE_VIDEO_FILENAME_FORMAT, SAMPLE_FRAME_ROOT_DIRECTORY, videoFileIndex + 1 ) + 1;
        ...
        snprintf( pFilename, xFilenameSize, "%s/" SAMPLE_VIDEO_FILENAME_FORMAT, SAMPLE_FRAME_ROOT_DIRECTORY, videoFileIndex + 1 );
...
#endif
        printf( "video t:%lu\n", pStreamData->videoFrameTimestamp );

        pStreamData->pFrameData = pFrameBuf;
        pStreamData->uFrameLen = uFrameLen;
        pStreamData->isKeyFrame = (videoFileIndex % DEFAULT_KEY_FRAME_INTERVAL ) == 0 ? true : false;
        if( pStreamData->isKeyFrame )
        {
            pStreamData->lastKeyFrameTimestamp = pStreamData->videoFrameTimestamp;
        }

        videoFileIndex = (videoFileIndex + 1 ) % H264_SAMPLE_FRAMES_SIZE;
    }
    else
    {
        pStreamData->audioFrameTimestamp = getEpochTimestampInMs();
        pStreamData->trackId = MKV_AUDIO_TRACK_NUMBER;

        /* Load frame from file */
        xFilenameSize = snprintf( NULL, 0, "%s/" SAMPLE_AUDIO_FILENAME_FORMAT, SAMPLE_FRAME_ROOT_DIRECTORY, audioFileIndex + 1 ) + 1;
        pFilename = ( char * )malloc( xFilenameSize );
        if( pFilename == NULL )
        {
            printf("OOM: audio frame filename\r\n");
            return KVS_STATUS_NOT_ENOUGH_MEMORY;
        }
        snprintf( pFilename, xFilenameSize, "%s/" SAMPLE_AUDIO_FILENAME_FORMAT, SAMPLE_FRAME_ROOT_DIRECTORY, audioFileIndex + 1 );
        uFrameLen = readFile( pFilename, pFrameBuf, uFrameBufSize );
        free( pFilename );

        printf( "audio t:%lu\n", pStreamData->audioFrameTimestamp );
        pStreamData->pFrameData = pFrameBuf;
        pStreamData->uFrameLen = uFrameLen;
        pStreamData->isKeyFrame = false;

        audioFileIndex = ( audioFileIndex + 1 ) % AAC_SAMPLE_FRAMES_SIZE;
    }
...
static int initializeMkvHeader( StreamData_t *pStreamData )
{
    ...
    Mkv_generateAacCodecPrivateData(AAC_LC, AUDIO_FREQUENCY, AUDIO_CHANNEL_NUMBER, pAudioCpdData, sizeof( pAudioCpdData ) );
    ...
    memset( &audioTrackInfo, 0, sizeof( AudioTrackInfo_t) );
    audioTrackInfo.pTrackName = DEFAULT_AUDIO_NAME;
    audioTrackInfo.pCodecName = AUDIO_CODEC_NAME;
    audioTrackInfo.uFrequency = AUDIO_FREQUENCY;
    audioTrackInfo.uChannelNumber = AUDIO_CHANNEL_NUMBER;
    audioTrackInfo.pCodecPrivate = pAudioCpdData;
    audioTrackInfo.uCodecPrivateLen = MKV_AAC_CPD_SIZE_BYTE;
...
static int updateFragmentData( StreamData_t *pStreamData )
{
    ...
    if( pStreamData->isKeyFrame )
    {
        timestamp = pStreamData->videoFrameTimestamp;
        deltaTimestamp = 0;
    }
    else
    {
        timestamp = 0;
        deltaTimestamp = ( uint16_t )( pStreamData->videoFrameTimestamp - pStreamData->lastKeyFrameTimestamp );
    }
                                         ...
                                         uint8_t * customData )
{
    ...
    if( pStreamData->streamState == STREAM_STATE_LOAD_FRAME )
    {
        loadFrame( pStreamData );
        pStreamData->streamState = STREAM_STATE_SEND_PACKAGE_HEADER;
    }
...
static int initializeStreamData( StreamData_t *pStreamData )
{
    ...
    if( retStatus == KVS_STATUS_SUCCEEDED )
    {
        ...
        pStreamData->isFirstFrame = false;
        pStreamData->isKeyFrame = false;
        pStreamData->videoFrameTimestamp = 0;
